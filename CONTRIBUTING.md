...
Thanks for your interest in cordon — a hardened, ephemeral, network-isolated command sandbox. It is one component of the
...
```bash
git clone https://github.com/Barnett-Studios/cordon
cd cordon
# shell + python; a container runtime (Docker) is needed only for the live-smoke tests
pip install pytest
```
...
|---|---|---|
| Shell lint | `shellcheck bin/*.sh` | clean |
| Tests | `pytest` | all pass |
| Image | `docker build -f docker/runtime.Dockerfile .` | builds (if the image changed) |
...
cordon is defined by its [`CONTRACT.md`](CONTRACT.md): the interface, the invariants, and the
...
[security advisory](https://github.com/Barnett-Studios/cordon/security/advisories/new) form so it can be
fixed before disclosure.

---

Questions that don't fit an issue? Open a
[discussion](https://github.com/Barnett-Studios/cordon/discussions). Thanks for contributing.
